...
class TokenAuth extends \yii\filters\auth\AuthMethod
{
    ...
    function __construct(){
    	if(Yii::$app->request->method == 'OPTIONS'){ // CORS跨域预检
		    exit;
	    }
    ...
    public function getAccessToken()
    {
        if (is_null($this->_accessToken)) {
            ...
            if ($methodsCount > 1) {
	            throw new HttpException(400, 'Only one method may be used to authenticate at a time (Auth header, POST or GET).');
//	            throw new Exception('Only one method may be used to authenticate at a time (Auth header, POST or GET).');
            } elseif ($methodsCount == 0) {
	            throw new HttpException(400, 'The access token was not found.');
//                throw new Exception('The access token was not found.', 1, 401);
            }
            ...
            if ($authHeader) {
                if (preg_match("/^Bearer\\s+(.*?)$/", $authHeader, $matches)) {
	                ...
	                throw new HttpException(400, 'Malformed auth header.');
//                    throw new Exception('Malformed auth header.');
                ...
                if ($postToken) {
                    if (! $request->isPost) {
                        throw new HttpException(400, 'When putting the token in the body, the method must be POST.');
                    }
                    // IETF specifies content-type. NB: Not all webservers populate this _SERVER variable
                    if ($request->contentType != 'application/x-www-form-urlencoded') {
	                    throw new HttpException(400, 'The content type for POST requests must be "application/x-www-form-urlencoded"');
